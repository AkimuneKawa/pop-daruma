...
- 規模は `constants.js` の `QTY`（=1200）で決まる。HANDOFF.md の値に比べて、**数量**（生産・客・在庫・容量・仕入れ・入荷上限）と**固定費**（家賃・給料・投資・初期資金）が QTY 倍、**単価**（売値1,500円・素材350円）は据え置き
- 来客は刻みごとにポアソン分布でまとめて抽選する。乾燥棚は `{c, n, ready}` の束で持つ。画面の絵（だるま・客）は QTY 個＝1体として描く
- 表示：金額は `yen()`（例：2,398万5,210円）、狭い欄は `yenShort()`（例：2,398万円／12.34億円）、個数は `cnt()`（例：7,200／2.4万）
- セーブキーは `popdaruma_rt_v3`。v1・v2 のセーブは `save.js` の `migrate` で変換して読み込む。`S` の構造を変えるときはキーを上げるか変換処理を追加する
- 色キー `sky` は表示名が「あお」でもセーブ互換のため変えない
- バランスを変えたら `npm run autoplay -- 200` で次を確認する：投資なし＝一人前（販売約20万個・年商 中央値約3.4億）、投資あり＝名工前後（年商 中央値約9億）、年商10億円は上手なプレイでたまに届く程度（自動プレイで数%）
